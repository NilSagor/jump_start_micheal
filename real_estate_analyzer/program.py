...
def get_data_file():
	...
	return os.path.join(base_folder, 'data', SacramentoRealEstateTranscations2008.csv)
...
def query_data(data):
	...
	two_bed_homes = (
		...
		for p in data #the set of process
		if annouce(p, '2-bedrooms.found{}'.format(p.beds)) and p.beds == 2
		)
	homes = []
	...
	ave_price = statistics.mean((annouce(p.price, 'price') for p in homes))
	ave_bath = statistics.mean((p.baths for p in homes))
	ave_sqft =  statistics.mean((p.sq__ft for p in homes))
	print("Average 2-bedrooms home is ${:,}, baths={}, sq ft= {:,}"
